...
try {
  ...
  const shellResponse = fetch('./app-shell.html?v=voice-20260913').then(async response => {
  ...
  const css=document.createElement('link');css.rel='stylesheet';css.href='./franchise.css?v=voice-20260913';document.head.append(css);
  document.body.replaceChildren(...Array.from(parsed.body.childNodes, node=>document.importNode(node,true)));
  const app=await import('./src/app.js?v=voice-20260913');appModule=app;shutdown=app.shutdownStudio;
