...
export function mountStudioTools({engine,getProfile,getTab,isBusy,isRecordLocked,stopHostInput,onLocks,setTab,onTrainingResult,getLatestRecording,getRecordElapsed,playEntry,downloadEntry,toast,recordToggle,getTrainingAsset=null}) {
  ...
  function paint(){
    ...
    set('boothTakeNote',take?`${Math.round(take.duration||0)}초 · ${take.captureSettings?.format==='wav-pcm'?`WAV ${take.captureSettings.containerBits}bit / ${take.captureSettings.outputSampleRate/1000}kHz`:take.mimeType?.includes('webm')?'WebM 원본':take.mimeType?.includes('wav')?'WAV 원본':take.mimeType||'원음'} · ${take.voiceMetrics?'음향 지표 저장됨':'원음 보관'} · ${take.unsaved?'기기 저장 실패':'이 회원의 기록에 저장됨'}`:'녹음 종료 → 음향 지표 저장 → 회원별 전후 비교');
