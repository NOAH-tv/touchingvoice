import { mountFrequencyDistribution, storedComposition } from './frequency-distribution.js?v=exam-20260916';
import { metricPriority } from '../research-policy.js';
import { ANALYZER_FIELDS, ANALYZER_BANDS } from './analyzer-metrics.js?v=monitor-fix-20260913';
import { analysisJson, datasetCsv } from './file-analysis-service.js?v=monitor-fix-20260913';
...
export function buildCoreAnalysisResult(entry){
 ...
 const groups=[
  {id:'pitch',title:'대표 음높이와 변동',fields:[coreField('대표 음높이 · F0',median,'Hz'),coreField('음높이 변동 · 표준편차',std,'Hz')],note:'이 녹음에서의 높이 변화입니다. 선율에 따라 변동이 커질 수 있습니다.'},
  {id:'observed-range',title:'이번 검사 음역',lead:rangeValid?`${noteFromHz(min)} → ${noteFromHz(max)}`:'—',fields:[coreField('관측 최저',rangeValid?min:null,'Hz'),coreField('관측 최고',rangeValid?max:null,'Hz')],note:'이번 녹음에서 검출된 범위이며, 낼 수 있는 전체 음역을 뜻하지 않습니다.'},
  {id:'level',title:'음량과 변화 폭',fields:[coreField('입력 레벨 중앙값',levelMedian,'dBFS'),coreField('음량 변화 폭 · DR',dynamicRange,'dB')],note:'같은 마이크·거리에서 비교하세요. 실제 음압이나 호흡 압력은 아닙니다.'},
  {id:'periodicity',title:'음성 주기성',fields:[coreField('배음 선명도 · CPP 추정',get('cpp_median'),'dB',2),coreField('배음·잡음 비 · HNR 추정',get('hnr_median'),'dB',2)],note:'자체 분석법의 참고값입니다. 같은 발성 과제에서 비교하세요.'},
  {id:'layers',title:'네 영역의 화면 반응',fields:layers,note:'4층위는 발성을 설명하는 시각 자료입니다. 개인 튜닝에 따라 움직이며 압력·근육 발달을 직접 측정하지 않습니다.'},
...
export function mountFileAnalysisView({getProfileId,getSessions,getJobs,upload,analyzeExisting,cancel,play,saveLabels,toast,beforePick,onSelectionChange=()=>{}}){
 const $=id=>document.getElementById(id);let selectedId=null,lastRenderKey='',mode='batch',holdEmpty=true,pendingUpload=null,selectionEpoch=0,observedProfileId=getProfileId();document.body.dataset.analysisMode='combined';$('liveAnalysisView').hidden=false;
 ...
 const coreHeading=makeText('div','batch-core-heading');coreHeading.append(makeText('h3','','코칭에 필요한 핵심 결과'),makeText('p','','음높이·이번 검사 음역·음량 변화 폭을 먼저 확인하세요. 전문 지표는 아래에서 펼쳐볼 수 있습니다.'));$('batchSummaryCards').before(coreHeading);$('batchSummaryCards').classList.add('batch-core-grid');$('batchSummaryCards').setAttribute('aria-label','음성 검사 핵심 결과');
 const researchDetails=$('batchDetails').closest('details');if(researchDetails){researchDetails.open=false;researchDetails.classList.add('batch-research-details');const title=researchDetails.querySelector('summary b'),hint=researchDetails.querySelector('summary small');if(title)title.textContent='전문 지표 · 시각화 참고값';if(hint)hint.textContent='CPP · HNR · 포먼트 · 4층위 · 전체 통계';researchDetails.addEventListener('toggle',()=>{if(researchDetails.open&&current()?.fileAnalysis)renderResearch(current());});const firstHeading=$('batchOriginalMetrics').previousElementSibling?.querySelector('h3');if(firstHeading)firstHeading.textContent='보관 중인 추가·탐색 지표';const formantHint=researchDetails.querySelector('.field-hint');if(formantHint)formantHint.textContent='추정 방법과 원본 수치를 함께 보관합니다. F1–F7로 표기된 기존 값은 고정 주파수 대역의 피크이며, 실제 포먼트 측정값으로 사용하지 않습니다. 빈칸은 측정 불가입니다.';}
 const savedDistribution=mountFrequencyDistribution($('batchBands'),{note:'선택한 녹음 전체의 평균 구성비 · 분석된 프레임별 에너지 비중의 평균입니다.'});
 const secondaryCore=makeText('div','batch-secondary-core');researchDetails?.querySelector('.exam-result-details-content')?.prepend(secondaryCore);
 ...
 function setMode(next){mode=next;document.body.dataset.analysisMode='combined';$('batchAnalysisView').hidden=false;$('liveAnalysisView').hidden=false;refresh();}
 ...
 function renderCore(entry){
  ...
  savedDistribution.update(storedComposition(entry.fileAnalysis?.additionalStats));secondaryCore.replaceChildren();
  ...
  const cards=core.groups.map(group=>{
   ...
   card.append(values,makeText('p','batch-core-card-note',group.note));card.title=group.note;return card;
  });
  $('batchSummaryCards').replaceChildren(...cards.slice(0,3));secondaryCore.replaceChildren(...cards.slice(3));
 }
 ...
 return {refresh,select,clearSelection,current,setMode,get selectionEpoch(){syncProfile();return selectionEpoch;},chooseFiles:pick,paint(){if(current()?.fileAnalysis)drawCharts(current().fileAnalysis);}};
}
