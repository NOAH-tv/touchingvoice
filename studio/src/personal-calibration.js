...
function compatibility(profile) {
  ...
  const result = { noiseGateDb: num(profile.global.noiseGateDb, -96, -6, '무음 기준'),
    inputGainDb: num(profile.global.inputGainDb, -24, 24, '입력 보정'), ...(profile.focusPreset?{focusPreset:profile.focusPreset}:{}), ...(profile.voicePreset?{voicePreset:profile.voicePreset}:{}), features: {} };
...
function checkedCompatibility(input) {
  ...
  if (input.focusPreset !== undefined && (input.focusPreset !== FEMALE_FOCUS_PRESET || input.voicePreset !== 'female')) fail('음향 대역 기준이 올바르지 않습니다.');
  return compatibility({ focusPreset: input.focusPreset, voicePreset: input.voicePreset, global: input, layers: Object.fromEntries(PERSONAL_LAYERS.map(key => [key, { feature: input.features[key] }])) });
}
function compatible(a, b, key) {
  return a?.focusPreset === b?.focusPreset && (a?.voicePreset||'male') === (b?.voicePreset||'male') && a?.noiseGateDb === b?.noiseGateDb && a?.inputGainDb === b?.inputGainDb && a?.features?.[key] === b?.features?.[key];
}
...
export function personalResponseRange(model, layerKey, features, profile) {
  ...
  const f = features || {}, current = { focusPreset: profile?.focusPreset, voicePreset: profile?.voicePreset, noiseGateDb: profile?.global?.noiseGateDb, inputGainDb: profile?.global?.inputGainDb,
