...
function clearPrivateState() { S.requestEpoch++; S.user = null; S.session = null; S.data = null; S.hq = null; S.branchId = ''; resetHistory(); $('#branchApplicationStatus').replaceChildren(); $('#branchApplyForm').reset(); $('#applyForm').reset(); $('#pendingError').hidden = true; $('#mainContent').replaceChildren(); $('#studioMount').replaceChildren(); $('#studioStudent').replaceChildren(); $('#branchSelect').replaceChildren(); $('#staffName').textContent = ''; $('#staffRole').textContent = ''; $('#staffAvatar').textContent = ''; closeModal(); document.title = '터칭보이스 · 로그인'; window.dispatchEvent(new CustomEvent('tv:session', { detail: null })); }
...
function renderModalRoute(route) {
  const routes = { student: studentDetail, 'hq-student': hqStudentDetail, lesson: lessonDetail, payment: paymentDetail, issue: issueDetail, exam: examDetail, coach: coachDetail };
...
async function submitModal(event) {
  ...
  try {
    if (form.id === 'studentForm') {
      ...
      const payload = { name: values.name.trim(), instructorUid: values.instructorUid, phone: values.phone.trim(), guardianPhone: values.guardianPhone.trim(), birthYear: values.birthYear ? Number(values.birthYear) : '', note: values.note.trim(), consent, active: id ? values.active === 'true' : true };
