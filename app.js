...
function showCachedBoot(boot) {
  ...
  finally { S.session = null; S.data = null; S.hq = null; S.studentExams = []; S.branchId = ''; }
}
function clearPrivateState({ preserveCache = false } = {}) { if (!preserveCache) clearBootCache(); S.requestEpoch++; S.user = null; S.session = null; S.data = null; S.hq = null; S.studentExams = []; S.branchId = ''; S.studentReturn = null; resetHistory(); $('#branchApplicationStatus').replaceChildren(); $('#branchApplyForm').reset(); $('#applyForm').reset(); $('#pendingError').hidden = true; $('#mainContent').replaceChildren(); $('#studioMount').replaceChildren(); $('#studioStudent').replaceChildren(); $('#branchSelect').replaceChildren(); $('#staffName').textContent = ''; $('#staffRole').textContent = ''; $('#staffAvatar').textContent = ''; closeModal(); document.title = '터칭보이스 · 로그인'; window.dispatchEvent(new CustomEvent('tv:session', { detail: null })); }
...
async function establishSession() {
  ...
  try {
    ...
    if (epoch !== S.requestEpoch) return;
    clearBootCache(); S.session = null; S.data = null; S.hq = null; S.studentExams = []; $('#mainContent').replaceChildren(); $('#portal').inert = false; $('#pendingEmail').textContent = S.user?.email || '로그인된 계정'; $('#pendingMessage').textContent = errorMessage(error); showScreen('pendingScreen'); loadBranchApplications();
  }
}
...
function renderModalRoute(route) {
  const routes = { student: studentDetail, 'hq-student': hqStudentDetail, lesson: lessonDetail, payment: paymentDetail, issue: issueDetail, exam: examDetail, 'student-exams': studentExamHistory, coach: coachDetail, 'staff-profile': staffProfile };
...
async function studentExamHistory(id) {
  const student=studentBy(id);if(!student)return;
  openModal(`${student.name} · 누적 검사 기록`, '<p role="status">서버 검사 기록을 불러오고 있습니다.</p>', 'STUDENT VOICE HISTORY');
  S.modalRoute={type:'student-exams',id};const epoch=S.modalEpoch,branchId=S.branchId,uid=S.user?.uid;
  try {
    const result=await call('exam.list',{studentId:id},branchId);
    if(epoch!==S.modalEpoch||branchId!==S.branchId||uid!==S.user?.uid||!$('#modal').open)return;
    if(!Array.isArray(result))throw new Error('검사 목록을 확인하지 못했습니다.');
    const exams=result.filter(row=>row.studentId===id).map(row=>({...row,branchId})).sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt));
    S.studentExams=exams;
    $('#modalBody').innerHTML=`<p class="section-caption">서버에 저장된 누적 검사 · ${exams.length}건</p>${table(['검사 일시','기본 주파수','HNR','기록'],exams.map(exam=>`<tr><td>${e(date(exam.createdAt))} ${e(time(exam.createdAt))}</td><td>${metric(exam,'f0')} Hz</td><td>${metric(exam,'hnr')} dB</td><td>${button('상세 지표','exam-detail',exam.id,'small ghost')}</td></tr>`),'아직 서버에 저장된 검사 기록이 없습니다.')}`;
  }catch(error){if(epoch===S.modalEpoch)$('#modalBody').innerHTML=notice(errorMessage(error),'amber')+button('다시 불러오기','student-exams',id);}
}
function examDetail(id) {
  const exam = (S.studentExams || []).find(row => row.id === id && row.branchId === S.branchId) || S.hq?.exams?.find(row => row.id === id); if (!exam) return; const rows = [];
...
async function handleAction(action, id, source) {
  ...
  if (action === 'student-exams') return studentExamHistory(id);
  ...
  if (action === 'exam-report') { const exam = (S.studentExams || []).find(row => row.id === id && row.branchId === S.branchId) || S.hq?.exams?.find(row => row.id === id); if (!exam) return; const url = new URL('./report.html', location.href); url.hash = new URLSearchParams({ exam: id, branch: exam.branchId }).toString(); return window.open(url.href, '_blank'); }
